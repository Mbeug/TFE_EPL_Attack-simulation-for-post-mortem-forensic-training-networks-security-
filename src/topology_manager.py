...
class TopologyManager:
    ...
    def create_n_node(self, n:int, template_name: str, switch_selected:str='in_lan'):
        ...
        if switch_selected == 'in_lan':
            ...
            for i in range(n):
                current_pc = self.nm.create_template_by_name(template_name,self.switch_services['x']+(len(self.list_services))*100,self.switch_services['y']+200)
                self.nm.add_file_to_node(current_pc["node_id"], "/etc/network/interfaces", self.net_config_dhcp)
    ...
    def create_DNS(self, personal_net_config_dns=None):
        ...
        if self.flag_have_nat and self.nat == None:
            self.nat = self.create_NAT()
            self.nm.link_nodes(self.nat["node_id"], self.switch_out["node_id"], [0,0],[0,self.get_switch_port(self.switch_out)])
        if self.switch_out == None:
            self.switch_out = self.nm.create_template_by_name("Ethernet switch", dns['x'] - 300,
                                                              dns['y'])

        self.nm.link_nodes(dns["node_id"], self.switch_out["node_id"], [0,0], [0, self.get_switch_port(self.switch_out)])
    ...
    def db_config(self, db):
        """
        Add some congfig to the specific db server
